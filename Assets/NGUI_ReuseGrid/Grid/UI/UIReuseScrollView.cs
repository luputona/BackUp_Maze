...
public class UIReuseScrollView : MonoBehaviour
{
    ...
    public enum ESPRING_DIRECTION
    {
        None,
        Left,
        Right
    }
    ESPRING_DIRECTION mSpringDirection = ESPRING_DIRECTION.None;
    ...
    public ESPRING_DIRECTION SpringDirection
    {
        get { return mSpringDirection; }
    }
    ...
    public void CheckSpringDirection()
    {
        Bounds b = bounds;
        Vector3 constraint = mPanel.CalculateConstrainOffset(b.min, b.max);
        constraint.y = 0f;
        ...
        if (constraint.magnitude > 1f)
        {
            if (constraint.x < 0f)
            {
                if (mSpringDirection == ESPRING_DIRECTION.None)
                {
                    //Debug.Log("Direction : Left");
                    mSpringDirection = ESPRING_DIRECTION.Left;
                }
            }
            else
            {
                if (mSpringDirection == ESPRING_DIRECTION.None)
                {
                    ...
                    mSpringDirection = ESPRING_DIRECTION.Right;
                }
            }
        }
        else
        {
            mSpringDirection = ESPRING_DIRECTION.None;
        }
    }
